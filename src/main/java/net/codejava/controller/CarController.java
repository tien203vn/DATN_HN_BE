...
public class CarController {
    ...
    public ResponseEntity<MetaResponse<MetaResponseDTO, List<CarResponseDTO>>> getListCarActiveForOwner(
        ...
        Integer ownerId =
                Integer.valueOf(jwtTokenUtil.getAccountId(servletRequest.getHeader(HttpHeaders.AUTHORIZATION)));
        return ResponseEntity.status(HttpStatus.OK).body(carService.getListCarActiveByOwner(requestDTO, ownerId));
    }

    @GetMapping(Endpoint.V1.Car.GET_LIST_IN_ACTIVE_FOR_OWNER)
    public ResponseEntity<MetaResponse<MetaResponseDTO, List<CarResponseDTO>>> getListCarInActiveForOwner(
            HttpServletRequest servletRequest, @ParameterObject MetaRequestDTO requestDTO) {
        Integer ownerId =
                Integer.valueOf(jwtTokenUtil.getAccountId(servletRequest.getHeader(HttpHeaders.AUTHORIZATION)));
